...
// GetConnection db
func GetConnection(name string) (database DatabaseConf, err error) {
	if dbConn == nil || len(dbConn) == 0 {
		return DatabaseConf{}, fmt.Errorf("Database connection is not initialized yet")
	}

	if db, ok := dbConn[name]; ok {
		var err error
		if db.Master == nil {
			err = errors.Wrapf(ErrDBNil, "%s DB nil", name)
		...
		database.Master = db.Master
		database.Slave = db.Slave

		return database, nil
	}

	return DatabaseConf{}, fmt.Errorf("Database connection " + name + " doesn't exist")
}
